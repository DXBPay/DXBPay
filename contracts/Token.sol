...
contract DXB is IERC20, Context, Ownable {
    ...
    uint public _taxFee = 5;
    uint public _restoreTaxFee;
    uint public _addLPFee = 2;
    uint public _restoreAddLPFee;
    uint public _liquidityFee = 50;
    ...
    bool public reductionInTax = true;
    bool public isSwapEnable = false;
    bool public inSwapAndLiquify;
    bool public inSwapAndLiquifyLP;
    
    uint public _rInSupply = 10000000000000e18; // 10 trillion.
    uint256 public _maxAmountPerTx = 5000000*10**18;
    uint256 public lastAddLPTimestamp = block.timestamp;
    uint256 public addLPDelay = 1 days;
    
    constructor (address _cycleBurn, address _teamVault, address _IAirDrop, address _IRewardPool, address[3] memory _feeWallet)  {
        name = 'DXB';
        symbol = 'DXB';
        decimals = 18;
        ...
        mint(msg.sender,345000000000000e18);
        mint(_teamVault,30000000000000e18);
     ...
     */
    function excludeFrom( address _wallet) public onlyOwner {
     ...
     * @param _wallet exclude wallet address.
     */
    function includeTo( address _wallet) public onlyOwner {
    ...
    function excludeFromHolderDeduction( address _wallet) public onlyOwner {
        require(!excludeFromDeduction[_wallet], "DXB :: excludeFrom deduction : already excluded");
        excludeFromDeduction[_wallet] = true;
    }
    
    function includeToHolderDeduction( address _wallet) public onlyOwner {
     ...
     */
    function _updateCycleburn( address _cycleBurn) public onlyOwner { CycleBurn = _cycleBurn; }
    
    function _updateRewardPool( address _rewardPool) public onlyOwner { rewardPool = _rewardPool; }
    
    function _updateIAirDropContract( address _IAirDropContract) public onlyOwner { IAirDropContract = _IAirDropContract; }
    
    function _updateTeamWaultContract( address _teamWaultContract) public onlyOwner { teamWaultContract = _teamWaultContract; }
    
    /** 
     * @dev Calls _updateCharityFee() function to update charity fee.
     * @param charityFee charity fee.
     */
    function _updateCharityFee( uint charityFee) public onlyOwner { _charityFee = charityFee; }
    
    /** 
     * @dev Calls _updateMarketingFee() function to update market fee.
     * @param marketingFee market fee.
     */
    function _updateMarketingFee( uint marketingFee) public onlyOwner { _marketFee = marketingFee; }
    
    /** 
     * @dev Calls _updateUseCaseFee() function to update usecase fee.
     * @param usecaseFee usecase fee.
     */
    function _updateUseCaseFee( uint usecaseFee) public onlyOwner { _useCaseFee = usecaseFee; }
    
    /** 
     * @dev Calls _updateCharityWallet() function to update charity address.
     * @param charityWallet charity address.
     */
    function _updateCharityWallet( address charityWallet) public onlyOwner { _charityFeeWallet = charityWallet; }
    
    /** 
     * @dev Calls _updateMarketingWallet() function to update market address.
     * @param marketingWallet market address.
     */
    function _updateMarketingWallet( address marketingWallet) public onlyOwner { _marketFeeWallet = marketingWallet; }
    
    /** 
     * @dev Calls _updateUseCaseWallet() function to update usecase address.
     * @param usecaseWallet usecase address.
     */
    function _updateUseCaseWallet( address usecaseWallet) public onlyOwner { _useCaseFeeWallet = usecaseWallet; }
    
    function _updateisSwapEnableStatus( bool _status) public onlyOwner { isSwapEnable = _status; }
    
    function _updateMaxAmountPerTx( uint maxAmountPerTx) public onlyOwner { _maxAmountPerTx = maxAmountPerTx; }
    
    function _updateAddLPDelay( uint _addLPDelay) public onlyOwner { addLPDelay = _addLPDelay; }
    ...
    function _transfer(address from, address to, uint256 value) private {
        ...
        if ((isSwapEnable) && (lastAddLPTimestamp.add(addLPDelay) <= block.timestamp) && (!inSwapAndLiquifyLP) && (from != uniswapV2Pair) && (contractTokenBalance > 0)){
